...
def get_columns_info(df, topN=3):
    '''
    カラム毎の「データ型」「欠損値数」「欠損率」「ユニーク数」「ユニーク率」等の情報をまとめたデータフレームを作成する関数
    ...
    for col, dtype in col_dtype_dict.items():
        #if  'int64' in [dtype] or 'float64' in [dtype]:          
        if  'int64' in str(dtype) or 'float64' in str(dtype):
    ...
    return summary_df
def get_columns_info_old(df, topN=3):
    '''
...
def col_data_in_dataframe(df):
    '''
    関数内容
    ・データフレームのカラム名毎の「データ型」「欠損値数」「欠損率」「ユニーク数」「ユニーク率」をまとめたデータフレームを作成する関数
    Input
    ・df：データフレーム
    関数使用方法
    ・col_data_in_dataframe(df)
    '''
...
def basic_check(df):
    ...
    ・df:データフレーム
    '''
    print('行と列の数は{}です。'.format(df.shape))
    print('--------------------------------------------')
    print('--------------------------------------------')
    print('各カラムの欠損値の数')
    print(df.isnull().sum())
...
def compare_bar_graph(df, df1, x_col, x1_col, y_col, y1_col, xticks_list, x_label, y_label, legend1, legend2, fontsize=18):
    ...
    関数使用方法
    ・compare_bar_graph(
    buy_df, sell_df, '格_b', '格_s', '度数', '度数', [i for i in range(1,11)], '格', '企業数', '企業数(b側)', '企業数(s側)'
    )
    '''
...
def complement_na_from_oneside(df, col1, col2, new_col):
    ...
    関数使用方法
    ・merge_df = complement_na_from_oneside(merge_df, 'コード_z', 'コード_c', '小分類')
...
def compare_bar_graph(df, df1, x_col, x1_col, y_col, y1_col, xticks_list, x_label, y_label, legend1, legend2, int_flag, fontsize=18, rotation=90):
    ...
    関数使用方法
    ・compare_bar_graph(
    f_buy_df, f_sell_df, '分類名_b', '分類名_s', '度数', '度数', None,
